...
test("auth=user mismatch ignores keyed publish", async ({
  page,
  request,
  namespace,
}) => {
  await page.goto(`/auth-mismatch?ns=${encodeURIComponent(namespace)}`);
  await expect(page.getByTestId("counter-value")).toHaveText("0");

  await request.post("/api/counter/increment-auth", { data: { namespace } });

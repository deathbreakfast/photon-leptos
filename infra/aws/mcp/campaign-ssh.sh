...
SSH_KEY="${SSH_KEY:-$HOME/.ssh/continuum-bench.pem}"
...
remote_bootstrap_server() {
  ...
  ssh_cmd "$host" "sudo mkdir -p '$INSTALL_DIR' && sudo tar -xzf /tmp/release.tar.gz -C '$INSTALL_DIR' && \
    sudo chmod +x '$INSTALL_DIR/photon-leptos-bench' '$INSTALL_DIR/photon-leptos-bench-server' '$INSTALL_DIR/photon-bench' && \
...
remote_bootstrap_loadgen() {
  local host="$1"
  ssh_cmd "$host" "sudo mkdir -p '$INSTALL_DIR' && sudo tar -xzf /tmp/release.tar.gz -C '$INSTALL_DIR' && \
    sudo chmod +x '$INSTALL_DIR/photon-leptos-bench' '$INSTALL_DIR/photon-bench'"
}
...
run_substrate() {
  ...
  ssh_cmd "$host" "cd '$INSTALL_DIR' && sudo mkdir -p reports && \
    sudo ./photon-bench run --experiment bm-p2 --storage sqlite --backend embedded --telemetry off \
      --hardware '$profile' --report ./reports/substrate-bm-p2-${profile}.json && \
    sudo ./photon-bench run --experiment bm-pl2 --storage sqlite --backend embedded --telemetry off \
...
run_pls_connection_campaign() {
  ...
  restart_server "$server_host"
  ssh_cmd "$lg_host" "sudo bash -c 'cd $install && env PHOTON_LEPTOS_BENCH_PROFILES=$install/profiles.json ./photon-leptos-bench run --experiment bm-pls1 --hardware $profile --server-url http://${server_private}:8080 --duration-secs $duration --report ./reports/bm-pls1-sqlite-embedded-${profile}.json'"
...
out = install / f'bm-pls0-sqlite-embedded-{profile}.json'
if (install / f'bm-pls0-rate100-{profile}.json').exists():
    base = json.loads((install / f'bm-pls0-rate100-{profile}.json').read_text())
else:
    base = {'experiment':'bm-pls0','matrix_slug':'sqlite-embedded-off-embedded-composite','scenario_id':'pls0-connection-sweep','hardware':profile,'profile_phase':1,'backend_id':'embedded','topology':'embedded-composite','telemetry':'off','storage':'sqlite','payload_bytes':256,'client_type':'synthetic'}
